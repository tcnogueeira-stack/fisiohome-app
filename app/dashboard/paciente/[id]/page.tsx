...
// Estrutura do paciente vindo da view
interface Paciente {
  id: string;
...
// Estrutura do registro financeiro vinculado ao atendimento
interface Financeiro {
  valor: number;
  forma_pagamento: string;
}

// Estrutura do atendimento com financeiro embutido
interface Atendimento {
  id: string;
  data_atendimento: string;
  descricao: string;
  tn007_financeiro?: Financeiro | Financeiro[];
}
...
export default function ProntuarioPaciente() {
  ...
  const carregarDados = async () => {
    ...
    setPaciente(pData as Paciente | null);
    ...
    if (pData) {
      ...
      setAtendimentos((aData as Atendimento[]) || []);
    }
  };
  ...
  const handleSalvarTudo = async (e: FormEvent) => {
    e.preventDefault();
                    ...
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '12px', borderBottom: '1px solid #F1F0EC', paddingBottom: '8px' }}>
                      ...
                      {fin && fin.valor > 0 ? (
                        <span style={{ backgroundColor: '#E2ECE9', color: '#2D5A53', padding: '4px 12px', borderRadius: '20px', fontSize: '0.85rem', fontWeight: 600 }}>
                          Recebido: R$ {Number(fin.valor).toFixed(2)} ({fin.forma_pagamento})
                        </span>
                      ) : (
